...
function FileCard({ file, onRegenerate, generateLabel, noExamsLabel }: { file: RepoFile; onRegenerate: (id: string) => void; generateLabel: string; noExamsLabel: string }) {
  const subjectStyle = SUBJECT_COLORS[file.subject] || { bg: "#f6f7f8", color: "#535862" };

  return (
    <div
      style={{ backgroundColor: "#fafdff", borderRadius: 24, border: "1px solid rgba(83,88,98,0.12)", padding: "20px", display: "flex", flexDirection: "column", gap: 14, transition: "box-shadow 0.18s ease, transform 0.18s ease", boxShadow: "rgba(4,69,144,0.04) 0px 4px 12px 0px" }}
    ...
    >
      ...
      {file.tags.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {file.tags.slice(0, 3).map((tag) => (
            <span key={tag} style={{ display: "flex", alignItems: "center", gap: 3, padding: "2px 8px", borderRadius: 9999, backgroundColor: "#f6f7f8", fontSize: 11, fontWeight: 500, color: "#535862" }}>
              <Tag size={9} />{tag}
            </span>
          ))}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto" }}>
        <span style={{ fontSize: 12, color: "#93979f", display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {file.examCount > 0 ? `${file.examCount} exam${file.examCount !== 1 ? "s" : ""} generated` : noExamsLabel}
        </span>
        <button
          onClick={() => onRegenerate(file.id)}
          style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: "#181d27", color: "#fff", borderRadius: 9999, padding: "7px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, transition: "background-color 0.12s ease" }}
...
export function Repository({ uploadedFiles, onRegenerateExam, onNavigateToExam }: RepositoryProps) {
  ...
  const filtered = allFiles.filter((f) => {
    const matchSearch = f.name.toLowerCase().includes(search.toLowerCase()) || f.subject.toLowerCase().includes(search.toLowerCase()) || f.tags.some((tag) => tag.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div style={{ backgroundColor: "#ebf5ff", minHeight: "calc(100vh - 64px)", fontFamily: "'Geist','Inter',sans-serif" }} className="px-4 py-10">
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div className="mb-8">
          <h1 style={{ fontFamily: "'Inter',sans-serif", fontSize: 28, fontWeight: 700, color: "#0a0d12", letterSpacing: "-0.04em" }}>{t("repo_title")}</h1>
          <p style={{ fontSize: 14, color: "#93979f", marginTop: 6, letterSpacing: "-0.01em" }}>{t("repo_desc")}</p>
        </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {subjects.map((s) => (
              <button
                ...
                style={{ padding: "9px 16px", borderRadius: 9999, border: filterSubject === s ? "none" : "1px solid rgba(83,88,98,0.15)", cursor: "pointer", fontSize: 13, fontWeight: 500, backgroundColor: filterSubject === s ? "#181d27" : "#fafdff", color: filterSubject === s ? "#fff" : "#535862", transition: "background-color 0.12s ease, color 0.12s ease, border-color 0.12s ease, scale 0.15s ease-out" } as React.CSSProperties}
              >
                {s}
              </button>
            ))}
        ...
        <div style={{ display: "flex", gap: 20, marginBottom: 24, flexWrap: "wrap" }}>
          <span style={{ fontSize: 13, color: "#93979f" }}><strong style={{ color: "#0a0d12", fontVariantNumeric: "tabular-nums" }}>{filtered.length}</strong> {t("repo_files")}</span>
          <span style={{ fontSize: 13, color: "#93979f" }}><strong style={{ color: "#0a0d12", fontVariantNumeric: "tabular-nums" }}>{allFiles.reduce((s, f) => s + f.examCount, 0)}</strong> {t("repo_exams_generated")}</span>
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 600, color: "#0a0d12" }}>{t("repo_no_files")}</p>
            <p style={{ fontSize: 13, color: "#93979f", marginTop: 4 }}>{t("repo_no_files_hint")}</p>
          </div>
        ) : (
