...
function QuestionCard({ q, index }: { q: Question; index: number }) {
  ...
  return (
    <div style={{ backgroundColor: "var(--card)", borderRadius: 20, border: "1px solid var(--border)", overflow: "hidden", marginBottom: 10 }}>
      <div
        role="button"
        tabIndex={0}
      ...
      >
        <span style={{ width: 26, height: 26, borderRadius: 8, backgroundColor: "var(--muted)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "var(--text-secondary)", flexShrink: 0 }}>
          {index + 1}
        </span>
        <p style={{ flex: 1, fontSize: 14, fontWeight: 500, color: "var(--foreground)", letterSpacing: "-0.01em" }}>{q.text}</p>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", flexShrink: 0 }}>{q.marks} pt{q.marks !== 1 ? "s" : ""}</span>
        ...
        <div style={{ padding: "0 18px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
          {q.options.map((opt, oi) => (
            <div key={oi} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 18, height: 18, borderRadius: q.type === "truefalse" ? 4 : "50%", border: `1.5px solid ${opt === q.answer ? "var(--brand)" : "rgba(83,88,98,0.25)"}`, backgroundColor: opt === q.answer ? "var(--secondary)" : "transparent", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {opt === q.answer && <div style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "var(--brand)" }} />}
              </div>
              <span style={{ fontSize: 13, color: opt === q.answer ? "var(--brand)" : "var(--text-secondary)", fontWeight: opt === q.answer ? 600 : 400 }}>{opt}</span>
              {opt === q.answer && <CheckCircle size={12} color="var(--brand)" />}
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ backgroundColor: "var(--background)", minHeight: "calc(100vh - 64px)", fontFamily: "'Geist','Inter',sans-serif" }} className="px-4 py-10">
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div className="mb-8">
          ...
          <p style={{ fontSize: 14, color: "var(--muted-foreground)", marginTop: 6, letterSpacing: "-0.01em" }}>{t("eg_desc")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            ...
            <div style={{ backgroundColor: "var(--card)", borderRadius: 24, border: "1px solid var(--border)", padding: 24 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <h3 style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)", letterSpacing: "-0.02em" }}>{t("eg_source")}</h3>
                <span style={{ fontSize: 11, color: selectedFiles.length === MAX_MATERIALS ? "#e05a00" : "var(--muted-foreground)", fontWeight: 500 }}>
                  {selectedFiles.length}/{MAX_MATERIALS}
                </span>
              </div>
              {availableFiles.length === 0 ? (
                <p style={{ fontSize: 13, color: "var(--muted-foreground)" }}>{t("eg_no_files")}</p>
              ) : (
                <>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ fontSize: 13, fontWeight: 500, color: "var(--foreground)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: 0 }}>
                                {file.name}
                              </p>
                              <span style={{ fontSize: 11, fontWeight: 600, color: style.color }}>{file.subject}</span>
                            </div>
                            <button
                              type="button"
                              onClick={() => removeFile(file.id)}
                              aria-label={t("eg_picker_remove")}
                              style={{
                                width: 26,
                                height: 26,
                                borderRadius: 8,
                  ...
                  >
                    <FolderOpen size={15} />
                    {selectedDocs.length > 0 ? t("eg_picker_change") : t("eg_picker_open")}
                  </button>
                  {selectedFiles.length === MAX_MATERIALS && (
                    ...
                    {difficultyKeys.map(({ val, label }) => (
                      <button key={val} onClick={() => setDifficulty(val)}
                        style={{
                          ...
                          color: difficulty === val ? "var(--primary-foreground)" : "var(--text-secondary)",
                    ...
                    {([
                      ...
                      <button key={val} onClick={() => setExamLanguage(val)}
                        style={{
                          ...
                          color: examLanguage === val ? "var(--primary-foreground)" : "var(--text-secondary)",
                          transition: "background-color 0.12s ease, color 0.12s ease",
                        }}>
                        {label}
                      </button>
            ...
            {/* Grading */}
            <div style={{ backgroundColor: "var(--card)", borderRadius: 24, border: "1px solid var(--border)", padding: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)", letterSpacing: "-0.02em", marginBottom: 4 }}>{t("eg_grading")}</h3>
              <p style={{ fontSize: 12, color: "var(--muted-foreground)", marginBottom: 16 }}>{t("eg_grading_hint")}</p>
              <div className="grid grid-cols-3 gap-3">
                {[
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)" }}>{t("eg_total_marks")}</span>
                <span style={{ fontSize: 18, fontWeight: 700, color: "var(--foreground)", fontVariantNumeric: "tabular-nums" }}>{totalMarks}</span>
              </div>
            </div>
            ...
            <button
              ...
              style={{
                backgroundColor: generating || selectedFiles.length === 0 || disabled ? "var(--muted-foreground)" : "var(--primary)",
              ...
              onMouseLeave={(e) => { if (!generating && selectedFiles.length > 0) e.currentTarget.style.backgroundColor = "var(--primary)"; }}
            >
            ...
            {disabled && onUpgrade && (
              <div style={{ marginTop: 12 }}>
                <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 8, lineHeight: 1.4 }}>
                  {t("eg_upgrade_hint")}
                </p>
                <button
                  type="button"
          ...
          <div className="md:col-span-3">
            {generating && (
              <div style={{ backgroundColor: "var(--card)", borderRadius: 24, border: "1px solid var(--border)", padding: 48, textAlign: "center" }}>
                ...
                <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 18, fontWeight: 600, color: "var(--foreground)", letterSpacing: "-0.03em", marginBottom: 8 }}>{t("eg_ai_loading")}</p>
                <p style={{ fontSize: 13, color: "var(--muted-foreground)" }}>{t("eg_ai_subtext")}</p>
                <div style={{ marginTop: 24, height: 4, backgroundColor: "var(--muted)", borderRadius: 9999, overflow: "hidden", maxWidth: 240, margin: "24px auto 0" }}>
            ...
            {!generating && !generatedExam && (
              <div style={{ backgroundColor: "var(--card)", borderRadius: 24, border: "1px solid var(--border)", padding: 48, textAlign: "center" }}>
                <div style={{ width: 64, height: 64, borderRadius: 20, backgroundColor: "var(--muted)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                  <FileText size={28} color="var(--muted-foreground)" />
                </div>
                <p style={{ fontSize: 16, fontWeight: 600, color: "var(--foreground)", letterSpacing: "-0.02em", marginBottom: 6 }}>{t("eg_empty_title")}</p>
                <p style={{ fontSize: 13, color: "var(--muted-foreground)" }}>{t("eg_empty_hint")}</p>
              </div>
            )}

            {!generating && generatedExam && (
              <div className="mffb-enter" style={{ backgroundColor: "var(--card)", borderRadius: 24, border: "1px solid var(--border)", overflow: "hidden" }}>
                {/* Exam header */}
                <div style={{ backgroundColor: "#9552e0", padding: "24px 24px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                    <div>
                      ...
                      <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: 18, fontWeight: 700, color: "var(--primary-foreground)", letterSpacing: "-0.03em", lineHeight: 1.2 }}>{generatedExam.title}</h2>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {onViewExam && (
                        <button onClick={() => onViewExam(generatedExam.id)} style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.25)", color: "var(--primary-foreground)", borderRadius: 9999, padding: "7px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                          {t("eg_open_full")}
                        </button>
                      )}
                      <button onClick={() => void onDownload?.(generatedExam.id, "pdf")} style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.15)", color: "var(--primary-foreground)", borderRadius: 9999, padding: "7px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                        <Download size={13} /> PDF
                      </button>
                      <button onClick={() => void onDownload?.(generatedExam.id, "docx")} style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.15)", color: "var(--primary-foreground)", borderRadius: 9999, padding: "7px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
