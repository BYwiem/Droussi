import { X } from "lucide-react";
import { useState } from "react";

interface LoginModalProps {
  onClose: () => void;
  onGoogleSignIn: () => void;
}

/**
 * Login dialog that pops over whatever is behind it (e.g. the landing page),
 * blurring the background. Plays a pop-in on mount and a softer pop-out on
 * close before unmounting via the parent's onClose.
 */
export function LoginModal({ onClose, onGoogleSignIn }: LoginModalProps) {
  const [closing, setClosing] = useState(false);

  const requestClose = () => {
    // With reduced motion the exit animation is disabled, so onAnimationEnd
    // never fires — close immediately instead.
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduceMotion) {
      onClose();
      return;
    }
    setClosing(true);
  };

  const handleAnimationEnd = () => {
    if (closing) onClose();
  };

  return (
    <div
      className="mffb-backdrop"
      data-closing={closing}
      onClick={requestClose}
      style={{
        position: "fixed",
        inset: 0,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        padding: 24,
        zIndex: 1000,
      }}
    >
      <div
        className="mffb-modal"
        data-closing={closing}
        onClick={(e) => e.stopPropagation()}
        onAnimationEnd={handleAnimationEnd}
        style={{
          position: "relative",
          backgroundColor: "#fafdff",
          borderRadius: 32,
          border: "1px solid rgba(83,88,98,0.15)",
          boxShadow: "rgba(4,69,144,0.2) 0px 24px 48px 8px",
          padding: "40px 40px",
          width: "100%",
          maxWidth: 440,
          fontFamily: "'Geist','Inter',sans-serif",
        }}
      >
        {/* Close button */}
        <button
          onClick={requestClose}
          aria-label="Close"
          style={{
            position: "absolute",
            top: 16,
            right: 16,
            width: 32,
            height: 32,
            borderRadius: "50%",
            backgroundColor: "#f6f7f8",
            border: "none",
            cursor: "pointer",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            transition: "background-color 0.12s ease, scale 0.15s ease-out",
          }}
          onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#ececf0")}
          onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "#f6f7f8")}
        >
          <X size={16} color="#535862" />
        </button>

        <h2
          style={{
            fontFamily: "'Inter',sans-serif",
            fontWeight: 600,
            fontSize: 22,
            color: "#0a0d12",
            letterSpacing: "-0.03em",
            marginBottom: 8,
          }}
        >
          Welcome back
        </h2>
        <p style={{ fontSize: 14, color: "#93979f", marginBottom: 28, letterSpacing: "-0.01em" }}>
          Sign in with your Google account to continue
        </p>

        {/* Google SSO Button */}
        <button
          onClick={onGoogleSignIn}
          style={{
            width: "100%",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            gap: 12,
            backgroundColor: "#181d27",
            color: "#ffffff",
            borderRadius: 9999,
            padding: "13px 24px",
            fontSize: 15,
            fontWeight: 500,
            letterSpacing: "-0.01em",
            border: "none",
            cursor: "pointer",
            transition: "background-color 0.15s ease, scale 0.15s ease-out",
          }}
          onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#2d3444")}
          onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "#181d27")}
        >
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M17.64 9.205c0-.639-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 01-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" fill="#4285F4"/>
            <path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 009 18z" fill="#34A853"/>
            <path d="M3.964 10.71A5.41 5.41 0 013.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 000 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
            <path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 00.957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z" fill="#EA4335"/>
          </svg>
          Continue with Google
        </button>
      </div>
    </div>
  );
}
